...
void GLWidget::draw()
{
 ...
 if (mZoom >= -0.5)
	glScalef(1+mZoom,1+mZoom,1+mZoom);
...
void GLWidget::resizeGL(int width, int height)
{
 ...
 mRange += mZoom;
 //glOrtho( -mRange , mRange, -mRange, mRange, mRange, -mRange );
 GLfloat x = (GLfloat)width / height;
 glFrustum(-x, x, -1.0, 1.0, 4.0+mRange, 25.0+mRange);
 glMatrixMode(GL_MODELVIEW);
}
...
{
	if (mZoom >= -0.5)
	mZoom += (((GLdouble)event->delta())/150.0);
	else mZoom =-0.5;
    updateGL();
}